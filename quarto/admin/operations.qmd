...
Indexation presets live in `config/indexation/`. ODC product definitions live in `config/products/pc/` (Planetary Computer), `config/products/cop/` (Copernicus CDSE), and `config/products/aef/` (AlphaEarth), with `metadata.product.source` set in each YAML.

List larger or slower products first in the `products` section of `INDEX_CONFIG` so parallel `make index` starts them before smaller ones (see `config/indexation/README.md`).

| Config | Source(s) | Products |
| ------ | --------- | -------- |
| `default.yaml` (default) | Planetary Computer + CDSE + AlphaEarth | 10 products: `aef_annual`, `s2_l2a` (PC), CLCplus (CDSE); S1 RTC, Landsat, NASADEM, ESA WorldCover, ESRI land cover (PC) |
| `s2_l2a_cop.yaml` | CDSE | `s2_l2a_cdse` |
| `alphaearth.yaml` | AlphaEarth | `aef_annual` |
